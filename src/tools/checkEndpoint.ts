...
import { Endpoint, EndpointCheckArgs } from "../types.js";

export const ENDPOINT_CHECKER_TOOL: Tool = {
  name: "check_endpoint",
  description:
    "Get detailed endpoint information including URL, request type, and all required/optional parameters with their types, samples, and descriptions. Provide a route path (e.g., '/prediction/daily-sun') or full URL.",
  inputSchema: {
    type: "object",
    properties: {
      route: {
...
export function findEndpoint(routePath: string): Endpoint | null {
...
function formatEndpointInfo(endpoint: Endpoint): string {
...
export function handleCheckEndpoint(args: unknown): { content: Array<{ type: string; text: string }> } {
  const { route } = args as EndpointCheckArgs;
