...
interface SearchFeaturesArgs {
  query: string;
  limit?: number;
}

type Scored = { route: string; description: string; score: number; snippet: string };
...
function scoreEntry(query: string, route: string, description: string): number {
  ...
  for (const t of qTokens) {
    if (routeTokens.includes(t)) overlap += 2; // stronger if in route path
    if (descTokens.includes(t)) overlap += 1;
  }

  // 2) Substring presence
  let substr = 0;
  if (routeNorm.includes(q)) substr += 2;
  if (descNorm.includes(q)) substr += 1;
...
function buildSnippet(description: string, query: string, maxLen = 180): string {
  const d = description.trim();
  const idx = d.toLowerCase().indexOf(query.toLowerCase());
  if (idx === -1) return d.length > maxLen ? d.slice(0, maxLen) + "…" : d;
  const start = Math.max(0, idx - Math.floor(maxLen / 3));
  const end = Math.min(d.length, start + maxLen);
  return (start > 0 ? "…" : "") + d.slice(start, end) + (end < d.length ? "…" : "");
}

export const SEARCH_FEATURES_TOOL: Tool = {
  name: "search_features",
  description:
    "Fuzzy search over route_descriptions by keywords (supports synonyms like 'planet details', 'mahadasha'). Returns most probable routes with scores and snippets.",
  inputSchema: {
    type: "object",
    properties: {
...
export function handleSearchFeatures(args: unknown) {
  const { query, limit } = (args as SearchFeaturesArgs) || {};
  const max = Math.min(Math.max(limit ?? 10, 1), 50);
  const entries: Scored[] = Object.entries(routeDescriptions).map(([route, description]) => {
