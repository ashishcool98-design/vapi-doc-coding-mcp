...
import { ResponseCheckArgs } from "../types.js";

export const RESPONSE_CHECKER_TOOL: Tool = {
  name: "check_response",
  description:
    "Get example API response data for a specific endpoint. Shows the actual response structure, sample data, and status codes. Provide a route path (e.g., '/prediction/daily-sun') or full URL.",
  inputSchema: {
    type: "object",
    properties: {
      route: {
...
function extractRoutePath(input: string): string {
...
export function findResponse(routePath: string): any {
  ...
  return categoryData[endpointName as keyof typeof categoryData];
}

function formatResponseInfo(responseData: any): string {
...
export function handleCheckResponse(args: unknown): { content: Array<{ type: string; text: string }> } {
  const { route } = args as ResponseCheckArgs;
